...
def migrate():
    ...
    logger.info("{} masternodes in 1.0\n".format(len(nodesDB_v10_nodes)))
    ...
    for node in botDB_v10_nodes:
        ...
        with nodesDB_v10 as db:
            ...
            tx = "{}-{}".format(userNode['txhash'], userNode['txindex'])
    ...
    logger.info("{} masternodes in 1.1".format(len(nodesDB_v11.getNodes())))
...
def getCollateralAge(txhash):
    ...
    try:
        ...
        result = subprocess.check_output(['curium-cli', 'getrawtransaction',txhash, '1'])
    ...
    try:
        ...
        result = subprocess.check_output(['curium-cli', 'getblock',blockHash])
